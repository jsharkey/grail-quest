...
files = [ "%04d.JPG" % (n) for n in range(5, 379) ]
lum_in = []
for dng in files:
	thumb = dng #"%s.thumb" % (dng)
	#if not os.path.isfile(thumb):
	#	subprocess.check_output(["tiffcp", dng, thumb], stderr=subprocess.STDOUT)
	stats = "%s.stats" % (dng)
	if not os.path.isfile(stats):
		print("identify -verbose %s > %s &" % (thumb, stats))
...
lum_out = savgol_filter(lum_in, 101, 3)

files = [ f.replace("JPG", "CR2") for f in files ]
...
for i in range(len(lum_in)):
	...
	print(lum_in[i], "\t", lum_out[i], "\t", exp_delta)
...
print("""find . -name "*.DNG" | parallel rawtherapee -o {}.jpg -p template.pp3 -p {}.pp3 -c {}""")
